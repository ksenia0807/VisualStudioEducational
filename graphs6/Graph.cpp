...
bool hasNegativeWeightCycle(const vector<Edge>& edges, const int& countVertex) {
    vector<int> distance(countVertex, 0); // Расстояния от начальной вершины до каждой вершины

    // Проходим по всем вершинам countVertex раз и обновляем расстояния
    for (int i = 0; i < countVertex; ++i) {
        for (const auto& edge : edges) {
    ...
    // Проверяем наличие циклов отрицательного веса с отрицательным циклом
    for (const auto& edge : edges) {
        int u = edge.start.index;
        int v = edge.end.index;
        int weight = edge.weight;
        if (distance[u] + weight < distance[v]) {
            return true; // Найден цикл отрицательного веса с отрицательным циклом
        }
    }

    return false; // Циклов отрицательного веса с отрицательным циклом не найдено
}

//----------------------------------------------------------------------------------------//

// Наличие отрицательных циклов с нулевым весом
bool hasNegativeZeroCycle(const vector<Edge>& edges, const int& countVertex) {
    std::vector<int> distance(countVertex, 0); // Расстояния от начальной вершины до каждой вершины

    // Проходим по всем вершинам numVertices - 1 раз и обновляем расстояния
    ...
    // Проверяем наличие отрицательных циклов с нулевым циклом
    for (const auto& edge : edges) {
        int u = edge.start.index;
        int v = edge.end.index;
        int weight = edge.weight;
        if (distance[u] + weight < distance[v] && distance[v] == 0) {
            return true; // Найден отрицательный цикл с нулевым циклом
        }
    }

    return false; // Отрицательных циклов с нулевым циклом не найдено
}

//----------------------------------------------------------------------------------------//

// Наличие положительных циклов, которые увеличивают общую длину пути
bool hasPositiveCycle(const vector<Edge>& edges, const int& countVertex) {
    std::vector<int> distance(countVertex, 0); // Расстояния от начальной вершины до каждой вершины
    ...
    // Проверяем наличие положительных циклов, которые увеличивают общую длину пути
    for (const auto& edge : edges) {
        int u = edge.start.index;
        int v = edge.end.index;
        int weight = edge.weight;
        if (distance[u] + weight < distance[v]) {
            // Проверяем достижимость вершины v из вершины u по положительному циклу
            std::vector<bool> visited(countVertex, false);
            visited[u] = true;

            while (!visited[v]) {
                for (const auto& edge : edges) {
                    if (edge.start.index == v) {
                        v = edge.end.index;
                        visited[v] = true;
                        break;
                    }
            ...
            return true; // Найден положительный цикл, увеличивающий общую длину пути
        }
    }

    return false; // Положительных циклов не найдено
}
//----------------------------------------------------------------------------------------//