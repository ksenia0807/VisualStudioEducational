...
int main()
{
	...
	if (hwnd != NULL)
	{
		...
		if (hdc != 0)
		{
			...
			Chandelier* BufChandelier[5];  //массив люстр
			...
			Man* BufObjects[5];    //массив конфликтующих объектов
			BufObjects[0] = &AMan;
			BufObjects[1] = &AMan1;
			...
			Man* pointerMan = &AMan; //инициализируем указатель на объекты классов с людьми
			Man* pointerMan1 = &AMan1; //инициализируем указатель на объекты классов с людьми
			Man* pointerManDop = &AManDop; //инициализируем указатель на объекты классов с людьми
			Man* SwOff = &ASwitchOff;  //инициализируем указатель на объекты классов с переключателями
			Man* SwOn = &ASwitchOn;        //инициализируем указатель на объекты классов с переключателями
			int flag = 0;  //флаг для определения включена ли люстра или нет
			while (1)
			{
				...
				for (int i = 0; i < 5; i++)  //цикл по объектам
				{
