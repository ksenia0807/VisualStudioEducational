...
void CoordinateDescent(double current_x1, double current_x2, double current_x3);
int main()
{
...
void CoordinateDescent(double current_x1, double current_x2, double current_x3)
{
	...
	cout << setw(11) << left << " Итерация" << setw(12) << left << "x1 " << setw(12)
		...
		while (grad(next_x1, next_x2, next_x3) > eps)
		{
			...
			step = StepCalculator(current_x1, current_x2, current_x3, p_x1, p_x2, p_x3); // получение значения шага для x1
			next_x1 = current_x1 + step * p_x1; //расчет следующего значения переменной х
			//подсчет значений элементов вектора L
			p_x1 = -grad_x1(next_x1, current_x2, current_x3);
			p_x2 = -grad_x2(next_x1, current_x2, current_x3);
			p_x3 = -grad_x3(next_x1, current_x2, current_x3);
			step = StepCalculator(next_x1, current_x2, current_x3, p_x1, p_x2, p_x3);
			next_x2 = current_x2 + step * p_x2;
			p_x1 = -grad_x1(next_x1, next_x2, current_x3);
			p_x2 = -grad_x2(next_x1, next_x2, current_x3);
			p_x3 = -grad_x3(next_x1, next_x2, current_x3);
			step = StepCalculator(next_x1, next_x2, current_x3, p_x1, p_x2, p_x3);
