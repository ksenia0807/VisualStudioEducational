...
void ProgFletcherReeves(double current_x1, double current_x2, double current_x3)
{
	...
	cout << setw(11) << left << " Итерация" << setw(12) << left << "x1 " << setw(12)
		...
		while (grad(next_x1, next_x2, next_x3) > eps)
		{
			...
			p_next_x1 = -grad_x1(next_x1, next_x2, next_x3) + (pow(grad(next_x1,
				next_x2, next_x3), 2) / pow(grad(current_x1, current_x2, current_x3), 2)) * p_x1;
			p_next_x2 = -grad_x2(next_x1, next_x2, next_x3) + (pow(grad(next_x1,
				next_x2, next_x3), 2) / pow(grad(current_x1, current_x2, current_x3), 2)) * p_x2;
			p_next_x3 = -grad_x3(next_x1, next_x2, next_x3) + (pow(grad(next_x1,
				next_x2, next_x3), 2) / pow(grad(current_x1, current_x2, current_x3), 2)) * p_x3;
			//подсчет шага в текущей итерации
			step = StepCalculator(next_x1, next_x2, next_x3, p_next_x1, p_next_x2,
				p_next_x3);
